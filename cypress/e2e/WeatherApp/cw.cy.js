...
describe('Hava Durumu Bilgileri', () => {
    ...
    it('displays weather information correctly', () => {
      ...
      cy.get('.temperature').should('have.text', '29°C')
      cy.get('.details > :nth-child(2)').should('contain.text', '32°C')
      cy.get('.details > :nth-child(4)').should('contain.text', '65%')
      ...
      cy.get('.accordion > :nth-child(1) > .accordion__heading > .accordion__button > .daily-item > .day').should('have.text', 'Thursday')
      cy.get('.accordion > :nth-child(1) > .accordion__panel > .daily-details-grid > :nth-child(2)').should('have.text', 'Humidity:32')
      cy.get('.accordion > :nth-child(7)').should('be.visible')
      cy.get('.accordion > :nth-child(7) > .accordion__heading').click()
      cy.get('.accordion > :nth-child(7) > .accordion__heading > .accordion__button > .daily-item > .day').should('have.text', 'Wednesday')
      cy.get('.accordion > :nth-child(7) > .accordion__panel > .daily-details-grid > :nth-child(4)').should('have.text', 'Wind speed:6.92 m/s')
    })
  })
